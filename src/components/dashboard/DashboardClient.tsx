...
import { prefetchForecastData } from '@/services/forecastService';
import { Skeleton } from "@/components/ui/skeleton";

type ChartType = "drought" | "flood" | "all";

// Separate component for chart rendering to improve performance
const ForecastTrendChart = ({ 
  ...
  const getTrendsData = () => {
    ...
    if (chartType === "all") {
      return weeklyForecast.forecasts.map((item: any) => ({
        date: item.date,
...
export default function DashboardClient() {
  ...
  const handleForecastClick = () => {
    // Prefetch forecast data when user hovers or clicks the forecast button
    prefetchForecastData().catch(err => console.error('Error prefetching data:', err));
  };
    ...
    <>
      ...
      <div className="flex justify-end px-4">
        ...
        <Button 
          ...
          variant="default" 
          size="sm"
          onMouseEnter={handleForecastClick}
